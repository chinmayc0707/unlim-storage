...
async def download_file(client: TelegramClient,
    ...
    if hasattr(location, 'size'):
        size = location.size
    elif hasattr(location, 'document') and location.document and hasattr(location.document, 'size'):
        size = location.document.size
    elif hasattr(location, 'photo') and location.photo and hasattr(location.photo, 'sizes') and location.photo.sizes:
        size = location.photo.sizes[-1].sizes[-1] if hasattr(location.photo.sizes[-1], 'sizes') else getattr(location.photo.sizes[-1], 'size', 0)
    else:
        # fallback for Message objects
        size = getattr(location, 'file', None)
        if size:
