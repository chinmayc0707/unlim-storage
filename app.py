...
def login():
    ...
    if success:
        ...
        # Handle AuthRestartError (common when session is invalid/stale)
        if error and "AuthRestartError" in str(error): 
            print(f"AuthRestartError detected for {phone}. Cleaning up and retrying...")
            
            # 1. Disconnect the existing manager explicitly
            try:
                manager.client.loop.run_until_complete(manager.client.disconnect())
                manager.loop.close()
            except Exception as e:
                print(f"Error disconnecting manager during retry: {e}")

            # 2. Remove from active managers
            ...
            # 4. Delete the stale session file
            session_path = os.path.join('sessions', f"user_pending_{phone}.session")
            if os.path.exists(session_path):
                try:
                    os.remove(session_path)
                except Exception as e:
                    print(f"Failed to delete stale session: {e}. Trying to rename...")
                    try:
                        # If delete fails, rename to move it out of the way
                        timestamp = int(time.time())
                        os.rename(session_path, session_path + f".trash_{timestamp}")
                    except Exception as e2:
                        print(f"Failed to rename stale session: {e2}")

            # 5. Retry once
            # Since manager was deleted, we get a fresh one
            manager = get_manager(f"pending_{phone}")
            success, error = manager.send_code(phone)
            if success:
                return jsonify({'status': 'code_sent', 'phone': phone})
...
def verify():
    ...
    if success:
        ...
        # Explicitly delete manager reference to help generic GC release file handles
        del manager
        import gc
        ...
        if os.path.exists(old_session):
            if os.path.exists(new_session):
                try:
                    ...
                    # If we can't remove the destination, we might have issues, but let's try rename anyway
                    pass
            ...
            if not renamed:
                 # Fallback: Try to COPY the file if rename fails (source might be read-locked but readable)
                 try:
                     print(f"Rename failed for {old_session}, trying copy...")
                     shutil.copy2(old_session, new_session)
                     # If copy succeeds, try to delete the old file (best effort)
                     try:
                         os.remove(old_session)
                     except:
                         print(f"Warning: Could not delete pending session file {old_session} after copy. It will be left as garbage.")
                 except Exception as e:
                     # If copy also fails, then we really have a problem
                     return jsonify({'error': f"Failed to rename or copy session file: {str(e)}"}), 500
