...
def verify():
    ...
    if success:
        ...
        old_session = os.path.join(BASE_DIR, 'sessions', f"user_pending_{phone}.session")
        new_session = os.path.join(BASE_DIR, 'sessions', f"user_{user.id}.session")
...
def logout():
    ...
    if manager:
        ...
        try:
            os.remove(os.path.join(BASE_DIR, 'sessions', f"user_{get_current_user_id()}.session"))
        except Exception as delete_error:
            print(f"Warning: Could not delete session file: {delete_error}")
